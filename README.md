...
(1) 상위권 영화 데이터를 가져와 놓고, 정작 가장 높은 평점의 영화 정보를 보여주지 않은 것 같다는 생각을 뒤늦게 하게 되었습니다. 
(2) 이 영화 데이터에는 평점이 2가지(IMDB_Rating, Meta_score) 종류였는데IMDb 평점을 프로젝트 주제로 결정했기 때문에 애초에 Meta_score 데이터를 삭제하고 작업했으면 더 확실한 결과물이 나왔을 것 같습니다. 두 매체의 평점을 한꺼번에 유저한테 보여줌으로써 오히려 유저에게 혼란을 줄 수 있기 때문입니다. 
(3) 유저에게 이 페이지에 대한 문제점 또는 개선점을 입력할 수 있는 공간을 주고, 그 내용을 저장하여 받고자 합니다. 이는 여러 프로젝트에 활용하고자 합니다.  
####  
추후에 개인적으로 위 개선점을 보완하여 작업을 하고 깃허브에 업로드할 예정입니다.   
